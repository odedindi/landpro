...
def predict_main(aoi_path):
    """this functions takes as input the json file fr4om frontend and returns the subpolygones with co2 metrics
    aoi: json file with area of interest from frontend
    """
    # download images from gee
    data_parent_path = os.path.join("..", "data", "raw")
    dataset_name = data.download_dataset(aoi_path, data_parent_path=data_parent_path, get_sent2=True,
                                         get_glc=False, get_ndvi=False)
